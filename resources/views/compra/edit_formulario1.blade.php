<div class="container col-md-12">
    <div class='panel panel-default'>	
        <div class="panel-body">
            <div class="row form-inline">
                ...
                <input type='text' class="form-control" id='Numero' name='Numero' placeholder='Numero' value={{$compra->Numero}} />
            </div>
        </div>
    </div>
    ...
    <div class='panel panel-default'>
        <div class="panel-body">
            <div class="row form-inline">
                <select id="CodigoProducto" name="CodigoProducto" class="CodigoProducto form-control">
                    <option value="">--Seleccione un producto--</option>
                    @foreach($productos as $producto)
                        <option value="{{$producto->CodProducto}}">{{$producto->Nombre}}</option>
                    @endforeach
                </select>  
                <input type='number' class="Cantidad form-control" size='5' id='Cantidad' name='Cantidad' placeholder='Cantidad' />
                <input type='number' class="Costo form-control" size='10' id='Costo' name='Costo' placeholder='Costo' />
                <input type='button' id='Insertar' name='Insertar' class=' Insertar btn-secundary' value='Insertar' />
            </div>
        </div>
    </div>
    <br />
    ...
    <div class='panel panel-default'>
        <div class="panel-body">
            <div class="table-responsive">
                <table id='Cuerpo' class=' Cuerpo table table-bordered'>
                    <thead>
                    ...
                    <table id='Totales' class='table table-condensed'>
                        <tbody>
                            <tr>
                                ...
                                <td><input type='text' readonly id='Total' name='Total' class='Total text-right form-control' value="{{$compra->Total}}"/></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <br />
    <input type='hidden' id='Detalle' name='Detalle' class="Detalle"/>
    <div style="text-align: center;">
        {!! Form::submit($textoBotonDeFormulario, ['class' => 'Guardar btn btn-success', 'id' => 'Guardar', 'name' => 'Guardar']) !!}
        {!! Form::close()!!}        
...
//        }); 
        ...
        $( ".Insertar" ).click(function(event) {
            event.preventDefault();
		var Cantidad = parseInt($("input:text[name=Cantidad]").val()) || 0;
                alert($("input:text[name=Cantidad]").val());
                var Costo = parseInt($('.Costo').val()) || 0;

                if( $('.CodigoProducto').val()=='0' ) {
                    ...
                    s += "<td style='display:none'>"+$('select#CodigoProducto').val()+"</td>";
                    s += "<td>"+$('.CodigoProducto option:selected').text()+"</td>";
