...
<section id='tools'>
    <ul class='breadcrumb' id='breadcrumb'>
      <li class='title'>Categorias de locales</li>
    </ul>
</section>
<div id='content'> 
    <div class="col-lg-12">
        <div class="col-lg-offset-8">
            {!! Form::open(['method' => 'GET', 'url' => 'categoria_locales', 'class' => 'navbar-form navbar-left', 'role' => 'search'])!!}
    ...
    <div class="desplegable">
        {!! Form::open(['method' => 'POST','route' => 'CategoriaLocal.store'])!!}
    ...
    <div>    
        <table border="1" class="table table-bordered table-hover table-responsive" id="mi-tabla">
            ...
            <tbody>
                <?php $cont = 0; ?>
                @forelse($categoria_locales as $categoria_local)            
                <tr>
                    {!! Form::model($categoria_local,['route' => ['CategoriaLocal.update',$categoria_local->CodCategoriaL],'method'=>'put']) !!}
                    <?php $cont++; ?>
                    <td id="<?php echo 'nombre'.$cont; ?>" class="nombre">{{$categoria_local->Nombre}}</td>
                    <td id="<?php echo 'editar'.$cont; ?>" class="editar">
                        <input type="text" name="nuevo" value="{{$categoria_local->Nombre}}">
                        {!! Form::submit('Guardar Nuevo', ['class' => 'btn btn-success']) !!}
                    </td> 
                    {!! Form::close()!!}
                    <td style="text-align: center;">
                        <div  class="row" style="display: inline-block; text-align: center;">
                            ...
                            <div style="display: inline-block; text-align: center;">
                                {!! Form::open(['method' => 'DELETE', 'route' => ['CategoriaLocal.destroy',$categoria_local->CodCategoriaL]]) !!}
                ...
                @empty
                <p>No existen categorias de locales registradas!</p>
                @endforelse
            </tbody>            
        </table>
    </div>
    <div class="row">
        {{$categoria_locales->links()}} 
    </div>
</div>
@endsection